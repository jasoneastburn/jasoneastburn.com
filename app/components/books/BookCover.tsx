...
export function BookCover({ image, alt }: { image: string; alt: string }) {
  ...
  const leftEdgeGradientClasses = clsx(
    'ml-4 h-full w-5',
    'absolute top-0 z-[5]',
  ...
  const rightEdgeGradientClasses = clsx(
  ...
  return (
    <div className="break-inside-avoid">
      <div className="relative">
        <div className={pageShadowClasses} />
        <div className={bookCoverClasses}>
          <Image src={image} alt={alt} width={1000} height={1500} className="w-full rounded-r-md" />
          <div className={leftEdgeGradientClasses} />
          <div className={rightEdgeGradientClasses} />
        </div>
      </div>
    </div>
  )
