...
export type ShelfType = 'all' | 'currently-reading' | 'read' | 'abandoned' | 'to-read'

export function ShelveSelect({ shelf }: { shelf: ShelfType }) {
  const { label, value: selectedValue } = SHELVES.find(({ value }) => value === shelf) || SHELVES[0]
        ...
        >
          <MenuItems
            className={clsx([
              ...
              'ring-opacity-5 ring-1 ring-black focus:outline-none',
            ])}
          >
